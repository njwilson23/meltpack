...
def solve_fluxdiv(h, u, v, α, options):
    """
    Arguments:
    ----------
    h, u, v: karta.RegularGrid, thickness and velocity components
    α: float, smoothing parameter
    ...
    εh_ = 5*np.ones_like(h)
    εu_ = 1*np.ones_like(h)
    εv_ = 1*np.ones_like(h)
...
def solve_veldiv(h, u, v, α, options):
    """
    Arguments:
    ----------
    h, u, v: karta.RegularGrid, thickness and velocity components
